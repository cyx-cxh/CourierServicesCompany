...
<div class="box2" panelTitle="查询" id="searchPanel">
    ...
    <form action="county/findByObj" id="queryForm" method="post">
        <table class="if_search_table">
            <tr>
                <td class="if_w80">区县名称：</td>
...
<div id="scrollContent" style="overflow-x:hidden;">
    <form id="myFormId" action="county/findByObj" method="post">
        <input type="hidden" value="${bean.name}" name="name" />
        <table class="tableStyle" mode="list">
            <tr>
                <th align="center" width="100">省份名称</th>
                <th align="center" width="100">城市名称</th>
                <th align="center" width="100">区县名称</th>
                <th align="center" width="200">操作</th>
            </tr>
            <c:forEach items="${list}" var="row" varStatus="l">
            <tr>
                <td align="center">${row.province.name}</td>
                <td align="center">${row.city.name}</td>
                <td align="center">${row.county.name}</td>
                <td align="center">
                    <a onclick="onEdit('${row.county.id}')"><span style="color: gold;">修改</span></a>
                    <a onclick="onDelete('${row.county.id}')"><span style="color: #00bd09">删除</span></a>
                </td>
            </tr>
            </c:forEach>
        </table>
...
<div style="height:35px;">
    ...
    <div class="float_right padding5">
        <pg:pager url="${ctx}/county/findByObj" maxIndexPages="5" items="${pagers.total}" maxPageItems="15"
