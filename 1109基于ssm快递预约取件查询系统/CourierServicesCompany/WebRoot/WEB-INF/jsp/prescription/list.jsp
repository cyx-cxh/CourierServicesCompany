...
<div class="box2" panelTitle="查询" id="searchPanel">
    ...
    <form action="prescription/findByObj" id="queryForm" method="post">
        <table class="if_search_table">
            <tr>
                <td class="if_w80">始发地：</td>
                <td class="if_w172">
                    <select selWidth="172" id="fjId" url="${ctx}/province/getProvinceList.json" prompt="请选择省份"
                            ...
                            selectedValue="${bean.provinceId}" colNum="2" colWidth="200" name="provinceId">
                    </select>
                </td>
                <td class="if_w172">
                    <select selWidth="172" id="pcsId" name="cityId"  prompt="选择城市"
                            selectedValue="${bean.cityId}"></select>
                </td>
                <td class="if_w80">目的地：</td>
                <td class="if_w172">
                    <select selWidth="172" id="fjId2" url="${ctx}/province/getProvinceList.json" prompt="请选择省份"
                            ...
                            selectedValue="${bean.endProvinceId}" colNum="2" colWidth="200" name="endProvinceId">
                    </select>
                </td>
                <td class="if_w172">
                    <select selWidth="172" id="pcsId2" name="endCityId"  prompt="选择城市"
                            selectedValue="${bean.endCityId}"></select>
                </td>
                <td>
...
<div id="scrollContent" style="overflow-x:hidden;">
    <form id="myFormId" action="prescription/findByObj" method="post">
        <table class="tableStyle" mode="list">
            <tr>
                <th align="center" width="150">始发地</th>
                <th align="center" width="150">目的地</th>
                <th align="center" width="100">天数</th>
                <th align="center" width="100">价格</th>
                <th align="center" width="100">操作</th>
            </tr>
            <c:forEach items="${list}" var="row" varStatus="l">
            <tr>
                <td align="center">${row.province.name}-${row.city.name}</td>
                <td align="center">${row.eprovince.name}-${row.ecity.name}</td>
                <td align="center">${row.prescription.time}</td>
                <td align="center">${row.prescription.jg}</td>
                <td align="center">
                    <a onclick="onEdit('${row.prescription.id}')"><span style="color: gold;">修改</span></a>
                    <a onclick="onDelete('${row.prescription.id}')"><span style="color: #00bd09">删除</span></a>
                </td>
            </tr>
            </c:forEach>
        </table>
...
<div style="height:35px;">
    ...
    <div class="float_right padding5">
        <pg:pager url="${ctx}/prescription/findByObj" maxIndexPages="5" items="${pagers.total}" maxPageItems="15"
                  export="curPage=pageNumber">
            <pg:param name="provinceId" value="${bean.provinceId}"/>
            <pg:param name="cityId" value="${bean.cityId}"/>
            <pg:first>
...
<script>
    ...
    function onDelete(rowid) {
        top.Dialog.confirm("确定要禁用该记录吗？", function () {
            //删除记录
            $.post("${ctx}/prescription/delete.json",
                    { "id": rowid },
                    function (result) {
